...
def Player_Info(Player_ID):
    data = Players.query.filter_by(Player_ID=Player_ID).first()
...
def Edit_Player(Player_ID):
    ...
    if request.method == 'POST':
        ...
        pla.Campaign=form.Campaign.data
        db.session.commit()
        return redirect("/Players")
    return render_template("Playerinput.html", form=form)
...
def Edit_Campaign(Campaign_ID):
    ...
    cam = Campaigns.query.filter_by(Campaign_ID=Campaign_ID).first()
    if request.method == 'POST':
        Campaign_Name=form.Campaign_Name.data
        Campaign_Setting=form.Campaign_Setting.data
        No_of_Players=form.No_of_Players.data
...
def Book_Info(Book_ID):
    data = Books.query.filter_by(Book_ID=Book_ID).first()
...
def Edit_Book(Book_ID):
    form = Update_Book()
    bo = Books.query.filter_by(Book_ID_ID=Book_ID).first()
    if request.method == 'POST':
        Book_Name=form.Book_Name.data
        Book_Contents=form.Book_Contents.data
        db.session.commit()
...
def Delete_Book(Book_ID):
    bo = Books.query.filter_by(Book_ID=Book_ID).first()
